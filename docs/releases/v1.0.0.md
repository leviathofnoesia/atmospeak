...
- **Speak and paste.** Hold or tap the hotkey, watch the cleaned live preview
  in the dock, and paste that hypothesis on release when coverage is good —
  without waiting on Final. Get exactly one paste into whatever application
  had focus.
...
flowchart LR
  ...
  release -->|yes_default| pastePreview[Paste preview]
  release -->|no_or_polish| finalize[Final + cleanup]
...
| `cargo test --manifest-path src-tauri/Cargo.toml --lib` | Pass — 77 tests |
| `bun run validation:paste-latency` | Pass — 74–86 ms / 300 ms budget |
| `bun run validation:native-ptt` | Pass — warm Vulkan `totalMs` 190–244 / 500 ms |

```mermaid
xychart-beta
    title "Warm release → paste (porcelain-moon / base.en)"
    x-axis ["Before: host ≤5s", "After: Vulkan stream"]
    y-axis "totalMs" 0 --> 2000
    bar [1718, 213]
```
...
| Typical `totalMs` | ~1250–1840 | **190–244** |
| SLO budget | ≤ 2000 ms | ≤ **500** ms |

Warm native PTT uses streaming Vulkan finalize (not the old ≤5 s host route).
Measured warm `totalMs` is **190–244 ms** under the **500 ms** release→paste
SLO; inject stays ≪ 150 ms. Paste harness contains-matches and trusts inject
when the target poll flakes. Full numbers are in the audit.
