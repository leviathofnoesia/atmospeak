...
The native push-to-talk harness did **not** pass on this base, and the cause is
not established. Across four attempts its paste target file stayed empty. On two
of those runs the paste target was the harness's own window and the text still
never arrived; on the other two, another process held foreground.

It is **not a 1.0.0 regression**: the same failure occurs on unmodified `main`,
and this release changes no application source. A product defect is also
unlikely — the paste-latency harness exercises the same clipboard, paste, and
focus-restore path against the same target window and passes here — but neither
of those is proof, and this gate should be treated as outstanding rather than
green until someone gets a clean run or finds the cause. Details in the audit.
